...
class CreateImageUploadModal extends React.Component {
  ...
  handleStartCompose(blueprintName, composeType, imageName, uploadSettings) {
    const upload = {
            ...
            <Form isHorizontal>
              {Object.keys(providerSettings[provider].auth).map(key => (
                <FormGroup
                  label={formatMessage({
                    id: `${provider}-auth`,
                    defaultMessage: providerSettings[provider].auth[key]
                  })}
                  fieldId={key}
                  key={key}
                >
              ...
              {Object.keys(providerSettings[provider].settings).map(key => (
                <FormGroup
                  label={formatMessage({
                    ...
                    defaultMessage: providerSettings[provider].settings[key]
                  })}
                  fieldId={key}
                  key={key}
                >
              ...
              {Object.keys(providerSettings[uploadService].auth).map(key => (
                <React.Fragment key={key}>
                  <TextListItem component={TextListItemVariants.dt}>
                    {formatMessage({
                      id: "review-settings",
                      defaultMessage: providerSettings[uploadService].auth[key]
                    })}
                  </TextListItem>
                  <TextListItem component={TextListItemVariants.dd}>{this.state.uploadSettings[key]}</TextListItem>
                </React.Fragment>
              ))}
              {Object.keys(providerSettings[uploadService].settings).map(key => (
                <React.Fragment key={key}>
                  <TextListItem component={TextListItemVariants.dt}>
                    {formatMessage({
                      id: "review-settings",
                      defaultMessage: providerSettings[uploadService].settings[key]
                    })}
                  </TextListItem>
                  <TextListItem component={TextListItemVariants.dd}>{this.state.uploadSettings[key]}</TextListItem>
                </React.Fragment>
