...
import AWSUploadAuth from "./AWSUploadAuth";
import AWSUploadSettings from "./AWSUploadSettings";
...
class CreateImageUploadModal extends React.Component {
  ...
  render() {
    ...
    const awsUploadStep = {
      name: "auth",
      component: <AWSUploadAuth uploadSettings={this.state.uploadSettings} setUploadSettings={this.setUploadSettings} />
    };
    ...
    const awsUpload = {
      name: "Upload to AWS",
      steps: [awsUploadStep, awsUploadSettings]
    };

    const steps = [imageStep, ...(showUploadAwsStep ? [awsUpload] : []), ...(showReviewStep ? [reviewStep] : [])];
